...
}
{
    ...
    foreach my $e (@$daten) {
        ...
        if($haus_lief_mit_solar) {
            $heizstatistik_tage->{$day}->[1] += $begleit_verbrauch + $wasser_wp_verbrauch;
            $heizstatistik_monate->{$month}->[1] += $begleit_verbrauch + $wasser_wp_verbrauch;
        }
    ...
    print "           |     Heizstab    |      Waermepumpe     | Begleitheiz |  Wasser-WP  | Heiz-Gesamt |   Wallbox   | Anzeil Solar \n";
    ...
    foreach my $day (scalar(@heizstab_tage) >= 14 ? @heizstab_tage[-14..-1] : @heizstab_tage) {
        ...
        my $gesamt_kwh = $kwh + $kwh_solar + $wp_kwh + $begleitheiz_kwh + $wasser_wp_kwh;
    ...
    print "        |   Heizstab   |    Waermepumpe    |  Begleitheiz |   Wasser-WP  |  Heiz-Gesamt  |    Wallbox    | Anteil Solar\n";
    ...
    foreach my $month (sort keys(%$heizstatistik_monate)) {
        ...
        my $kwh_gesamt = $kwh + $kwh_solar + $wp_kwh + $begleitheiz_kwh + $wasser_wp_kwh;
        ...
        printf(
            "%s | %5.1f  %5.2f | %5.1f  %5.2f   %2d | %5.1f  %5.2f | %5.1f  %5.2f | %5.1f  %6.2f | %5.1f  %6.2f | %5.1f  %5.2f\n",
            $month,
            $kwh, $kosten,
