...
//DynamicJsonDocument _hole_maximalen_strom_und_phase(Local::Config cfg, Local::ElektroAnlage elektroanlage) {
//	Local::SmartmeterLeser leser(cfg, web_client);
//	leser.daten_holen_und_einsetzen(elektroanlage);
//	// TODO das untere umwandeln
//
//  const String smartmeter_content = web_client.get(cfg.smartmeter_data_url);
...
//  b = s_data["Body"]["Data"][cfg.smartmeter_id]["channels"];
//  return _hole_maximalen_strom_und_phase_aus_json(b);
//}

//DynamicJsonDocument _hole_wechselrichter_daten(Local::Config cfg, Local::ElektroAnlage elektroanlage) {
//	// TODO dennach ganz außen setzen
//  Local::WechselrichterLeser leser(cfg, web_client);
//  leser.daten_holen_und_einsetzen(elektroanlage);
//
//	// DEPRECATED
...
//DynamicJsonDocument _hole_daten(Local::Config cfg, Local::ElektroAnlage elektroanlage) {
//  DynamicJsonDocument w_data1 = _hole_wechselrichter_daten(cfg, elektroanlage);
//  DynamicJsonDocument max_i1 = _hole_maximalen_strom_und_phase(cfg, elektroanlage);
//
...
Local::HTTPResponse _erzeuge_website(Local::Config cfg, Local::ElektroAnlage elektroanlage) {
	//Serial.println(ESP.getFreeHeap(),DEC);
//	_hole_wechselrichter_daten(cfg, elektroanlage);
...
//  }
  ...
  const String speicher_ladung = content_converter.formatiere_zahl(
  	false, cfg.speicher_groesse / 100 * elektroanlage.solarakku_ladestand_prozent
  );
  ...
  String html = "<html><head><title>Sonnenrech 19</title>"
    ...
    "  xhr.send();\n"
    "}\n"
    "setInterval(reload, " + (String) cfg.refresh_display_interval + " * 1000);\n"
    "</script>"
    "<style>"
    ...
    "<div style=\"display:none\"><span id=max_i_value>" + content_converter.formatiere_stromstaerke(elektroanlage.max_i_ma()) + "</span><span id=max_i_phase>" + elektroanlage.max_i_phase() + "</span></div>"
    "<table cellspacing=0>"
    "<tr><td class=label>Solar</td><td class=zahl>" + content_converter.formatiere_zahl(false, elektroanlage.solar_wh) + "</td><td class=\"label einheit\">W</span></td></tr>"
    "<tr><td class=label>+Akku<br/></td><td class=zahl>" + content_converter.formatiere_zahl(true, elektroanlage.solarakku_wh) + "</td><td class=\"label einheit\">W</span></td></tr>"
    "<tr><td class=label>+Netz</td><td class=zahl>" + content_converter.formatiere_zahl(true, elektroanlage.netz_wh) + "</td><td class=\"label einheit\">W</span></td></tr>"
    "<tr class=last><td class=label>=Last</td><td class=zahl>" +  content_converter.formatiere_zahl(false, elektroanlage.verbraucher_wh * -1) + "</td><td class=\"label einheit\">W</span></td></tr>"
    "<tr class=markerline><td colspan=3><span id=max_i></span><span id=marker>starte...</span><span id=fehler></span></td></tr>"
    "<tr class=\"speicher " + (elektroanlage.solarakku_ist_an ? "speicher_status" : "") + "\"><td class=label>Speicher</td><td class=zahl>" + speicher_ladung + "</td><td class=\"label einheit\">Wh</span></td></tr>"
    "</table>"
    "</body></html>";
