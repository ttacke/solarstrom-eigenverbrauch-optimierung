...
namespace Local {
	...
	public:
		void daten_holen_und_einsetzen(Local::ElektroAnlage& elektroanlage) {
			String content = web_client.get(cfg.smartmeter_data_url);
			elektroanlage.l1_strom_ma = round(_finde(
				(char*) "\"SMARTMETER_CURRENT_01_F64\" *: *([0-9.]+)[^0-9]",
				content
			).toInt() * 1000);
			elektroanlage.l2_strom_ma = round(_finde(
				(char*) "\"SMARTMETER_CURRENT_02_F64\" *: *([0-9.]+)[^0-9]",
				content
			).toInt() * 1000);
			elektroanlage.l3_strom_ma = round(_finde(
				(char*) "\"SMARTMETER_CURRENT_03_F64\" *: *([0-9.]+)[^0-9]",
				content
			).toInt() * 1000);
