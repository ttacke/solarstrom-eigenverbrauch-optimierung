...
namespace Local {
	...
	// Wieso knallts dann bei referenzen? Wird eins der Objekte zerstört? Muss man Referenzen überall mit & notieren?
		...
		char daten_json_format_buffer[128];
		int last_heartbeat_timestamp = 0;

		void _daten_json_append(const char* format, ...) {
			va_list args;
			va_start(args, format);
			int n = vsnprintf(daten_json_format_buffer, sizeof(daten_json_format_buffer), format, args);
			va_end(args);
			if(n <= 0) {
				return;
			}
			int len = n < (int) sizeof(daten_json_format_buffer) ? n : (int) sizeof(daten_json_format_buffer) - 1;
			web_writer.write(daten_json_format_buffer, len);
		}
	...
	public:
		...
		void zeige_daten_json() {
			if(last_heartbeat_timestamp == 0) {
				webserver.server.send(503, "text/plain", "Daten noch nicht bereit");
				return;
			}
			web_writer.init_for_write("application/json", -1);// chunked transfer encoding
			...
			_daten_json_append("\"stunden_balkenanzeige\":[");
			for(int i = 0; i < 12; i++) {
				_daten_json_append(
					i == 0 ? "%i" : ",%i",
					verbraucher.gib_stundenvorhersage_akku_ladestand_als_fibonacci(i)
				);
			}
			_daten_json_append("],");
			_daten_json_append(
			...
			_daten_json_append("\"tage_balkenanzeige\":[");
			for(int i = 0; i < 5; i++) {
				_daten_json_append(
					i == 0 ? "%i" : ",%i",
					wetter.gib_tagesvorhersage_solarstrahlung_als_fibonacci(i)
				);
			}
			_daten_json_append("],");
			_daten_json_append(
			...
			_daten_json_append(
				...
				(verbraucher.heiz_verdichter_aus_seit == 0 ? 0 : last_heartbeat_timestamp - verbraucher.heiz_verdichter_aus_seit)
			);
			_daten_json_append(
				"\"heiz_verdichter_laufzeit\":%i,",
				(verbraucher.heiz_verdichter_laeuft_seit == 0 ? 0 : last_heartbeat_timestamp - verbraucher.heiz_verdichter_laeuft_seit)
			);
			_daten_json_append(
